...
class CreateEventForm extends React.Component {
  constructor(props) {
    ...
    this.findTimezone = this.props.timezones.filter((timezone) => {
      return (
        timezone.locale === Intl.DateTimeFormat().resolvedOptions().timeZone
      );
    });
  ...
  getMin() {
    if (this.props.edit && new Date(this.state.start) < new Date()) {
      this.setState({
        min: this.convertDateToLocalAsJSON(new Date(this.state.start)).slice(
          0,
          16
        )
      });
  ...
  handleSubmit(e) {
    ...
    if (this.props.edit) {
      this.props.updateEvent(this.state).then((action) => {
        this.props.history.push(`/events/${action.event.id}/details`);
      });
    } else {
      this.props.createEvent(this.state).then((action) => {
        this.props.history.push(`/events/${action.event.id}/details`);
      });
    }
  }
  ...
  render() {
    ...
    return (
      <div id="create-event-form" className="create-form">
        <form
          id="create-event-info-form"
          className="create-info-form"
          onSubmit={this.handleSubmit}
        >
          <section className="info-section">
            <h1 className="create-header">
              <i className="far fa-edit create-event-form-icons"></i>Basic Info
            </h1>
            ...
            <label className="large-input-label">
              ...
              <input
                className="large-input"
                onChange={this.handleInputChange("organizer")}
          ...
          <section className="info-section">
            <h1 className="create-header">
              <i className="far fa-map create-event-form-icons"></i>Location
            </h1>
            <p className="create-description">
            ...
            <div id="create-event-radio-buttons" className="radio-buttons">
              <input
                ...
                name="location"
                value="VENUE"
                checked={this.state.location === "VENUE"}
              ...
              <input
                ...
                name="location"
                value="ONLINE"
                checked={this.state.location === "ONLINE"}
                onChange={this.handleRadioChange("location")}
              />
              ...
              <input
                ...
                name="location"
                value="TBA"
                checked={this.state.location === "TBA"}
                onChange={this.handleRadioChange("location")}
              />
          ...
          <section className="info-section">
            <h1 className="create-header">
              <i className="far fa-calendar-alt create-event-form-icons"></i>
              Date and time
            </h1>
            <p className="create-description">
            ...
            <div id="create-event-radio-buttons" className="radio-buttons">
              <input
                ...
                name="recurring"
                value="false"
                checked={this.state.recurring === "false"}
            ...
            <label id="timezone-label" className="">
              <p>TimeZone</p>
              <select
                name="timezones"
                id="timezones"
              ...
              >
                {this.props.timezones.map((timezone, i) => {
                  return (
                    <option value={timezone.zone} key={i}>
                      {timezone.zone}
                    </option>
                  );
                })}
