...
class Splash extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.myId) {
      this.props.clearMyEvents(this.props.myId);
    }
    //may not need to set state of featured collections
    //consider using the api to fetch featuredCollections without adding to redux store
    this.props
      .getFeaturedCollections()
      .then(() =>
        this.setState({ featuredCollections: this.props.featuredCollections })
      );
  ...
  componentWillMount() {
    this.props
      .getFollows()
      .then(() => this.setState({ follows: this.props.follows }));
  }
  changeEventList(e) {
    ...
    if (e.target.innerText === "All") {
      ...
      let category = this.props.categories.filter((cat) => {
        return cat.name === e.target.innerText;
      })[0];
      val = category.id;
      name = category.name;
    }
    ...
    // .then(() =>
    //   this.setState({ relevantEvents: this.props.events, popularIn: name })
    // );
  }
    ...
    return (e) => {
      ...
      if (direction === "forward") {
        if (
          ...
          this.state.featuredCollections.length - 1
        ) {
          currentCollection += 1;
  ...
  createFeaturedCollection() {
    ...
    let collection =
      this.state.featuredCollections[this.state.currentCollection];
    return (
      <div id="content">
  ...
  convertDateToLocalAsJSON(date) {
    return (date.toJSON(),
    new Date(date.getTime() - date.getTimezoneOffset() * 60000).toJSON()).slice(
      0,
      16
    );
  }
        ...
        <div id="favorite-collections">
          <div id="header">
            <span id="left">
              ...
              <p>
                Browse through some of the best collections in Online Events
                hand picked by people who know the area best.
              </p>
            </span>
            <span id="right">
              {/* <span></span> */}
              <span id="buttons">
                <button
                  className="endArrow"
                  id="left-arrow"
                  onClick={this.changeFeaturedCollections("backward")}
                >
                  ←
                </button>
                <button
                  ...
                  id="right-arrow"
                  onClick={this.changeFeaturedCollections("forward")}
                >
                  →
                </button>
              </span>
    ...
    return (
      <div id="splash">
        ...
        <button
          id="see-more-button"
          onClick={() =>
            this.props.history.push(`/events/browse/${this.state.popularIn}`)
          }
        >
          See more
        </button>
